...
const EN: Record<string, string> = {
  ...
  "例如：🙂 或 https://.../avatar.png": "For example: 🙂 or https://…/avatar.png",
  ...
  "Low - 省 token，推荐翻译使用": "Low — efficient; recommended for translation",
  ...
  "回复进行中允许排队新消息": "Queue new messages while a response is in progress",
  ...
  "Live - OpenAI 托管搜索，高上下文": "Live — OpenAI-hosted search with extended context",
  ...
  "已新增 OpenAI 配置，保存后生效。":
    "OpenAI account added. Save to apply it.",
  ...
  "复制成功": "Copied",
  "本轮仅发送题录/摘要信息": "Only bibliographic metadata and the abstract were sent",
  ...
  "手动触发 Zotero 官方笔记编辑器保存":
    "Save now in Zotero's note editor",
  "已写入": "Added",
  ...
  "联网已开启：Cached；点击可关闭": "Online is on: Cached. Click to turn it off.",
  ...
  "YOLO：本地工具无需审批直接执行":
    "YOLO: run local tools without approval",
  ...
  "请求已发送，等待首个流式事件": "Request sent; waiting for the first streamed event",
  ...
  "已取消": "Canceled",
  "失败": "Failed",
  "已收到思考过程，正在输出正文": "Reasoning received; generating the final response",
  "打开笔记": "Open note",
  "清空并保存当前条目的聊天记录":
    "Clear and save this item's chat history",
  "写入笔记": "Add to note",
  "导入笔记": "Add to note",
  "保存": "Save",
  "关闭": "Close",
  ...
  "当前窗口不支持文件选择器":
    "This window does not support the file picker",
  "导出失败": "Export failed",
  "导入失败": "Import failed",
...
function dynamicEnglish(value: string): string | null {
  const rules: Array<[RegExp, (...parts: string[]) => string]> = [
    ...
    [/^翻译 (\d+)\/(\d+) · (\d+)\/(\d+)$/, (paper, papers, paragraph, paragraphs) =>
      `Translating ${paper}/${papers} · ${paragraph}/${paragraphs}`],
    [/^第 (\d+) 段$/, (number) => `Paragraph ${number}`],
    [/^共 (\d+) 段，开始翻译。$/, (count) =>
      `Starting translation of ${count} paragraph(s).`],
    [/^已清除本篇 (\d+) 条段落翻译缓存，将重新翻译。$/, (count) =>
      `Cleared ${count} cached paragraph translation(s); translating again.`],
    [/^本篇有 (\d+) 段模型没有返回中文译文，已跳过并继续处理。$/, (count) =>
    ...
    [/^已带入 PDF 选区 (\d+) 字$/, (n) => `Included PDF selection: ${n} characters`],
    [/^(\d+) 未读 \/ (\d+) 总计$/, (unread, total) => `${unread} unread / ${total} total`],
    [/^加入\/移除 (.+)$/, (id) => `Add/remove ${id}`],
    [/^「新增文字」默认字号已保存为 (\d+)。$/, (size) => `Default Add Text font size saved as ${size}.`],
    [/^已识别 ChatGPT 登录(?:（(.+)）)?，检测到 (\d+) 个模型。点击保存账号配置生效。$/, (plan, count) => `ChatGPT sign-in detected${plan ? ` (${plan})` : ""}, with ${count} available models. Click Save account settings to apply.`],
    [/^正在测试 (\d+) 个新增\/变更配置；通过后保存\.\.\.$/, (count) => `Testing ${count} new or changed account(s); they will be saved after passing…`],
    ...
    [/^上次上传：(.+)$/, (value) => `Last upload: ${value === "未上传" ? "never" : value}`],
    [/^上次下载：(.+)$/, (value) => `Last download: ${value === "未下载" ? "never" : value}`],
    [/^翻译失败：(.+)$/, (detail) => `Translation failed: ${detail}`],
    [/^保存注释失败：(.+)$/, (detail) => `Could not save annotation: ${detail}`],
    [/^● 已完成 · 全文缓存$/, () => "● Complete · full-text cache"],
    [/^● 已完成 · 本地缓存$/, () => "● Complete · local cache"],
    [/^(.+) 下一段 · (.+) 上一段$/, (next, previous) => `${next} next · ${previous} previous`],
    [/^自定义提示词按钮；PDF 中按 (.+) 触发$/, (key) => `Custom prompt button; press ${key} in the PDF Reader`],
    [/^当前自定义按钮：(.+)$/, (labels) => `Current custom buttons: ${labels}`],
    [/^读取 Zotero 标注 (\d+) 条$/, (count) => `Read ${count} Zotero annotation(s)`],
    [/^检索 PDF: (.+)，返回 (\d+) 段$/, (query, count) => `Searched PDF for “${query}”; returned ${count} passage(s)`],
    [/^读取 PDF 范围 (\d+)-(\d+)$/, (start, end) => `Read PDF range ${start}–${end}`],
    [/^调用 Zotero 工具: (.+)$/, (tool) => `Calling Zotero tool: ${tool}`],
    [/^本地 Codex 请求超时：(.+)$/, (method) =>
      `Local Codex request timed out: ${method}`],
    [/^思考与上下文 · (.+)$/, (summary) => `Reasoning and context · ${uiText(summary, "en-US")}`],
    [/^已随本轮发送 PDF 选区 (\d+) 字(?:；自动附带附近上下文 (\d+) 字)?(?:；另保留最近上下文 (\d+) 条 \/ (\d+) 字)?$/, (selected, nearby, retained, retainedChars) => [
      `Sent PDF selection: ${selected} characters`,
      nearby ? `nearby context: ${nearby} characters` : "",
      retained ? `retained recent context: ${retained} item(s) / ${retainedChars} characters` : "",
    ].filter(Boolean).join("; ")],
    [/^已随本轮发送 Zotero 标注 (\d+) 条$/, (count) => `Sent ${count} Zotero annotation(s)`],
    [/^模型复用历史上下文 (\d+) 段 \/ (\d+) 字$/, (count, chars) => `Model reused ${count} historical context passage(s) / ${chars} characters`],
    [/^模型请求 PDF 字符范围 (\d+) 段 \/ (\d+) 字$/, (count, chars) => `Model requested ${count} PDF range(s) / ${chars} characters`],
    [/^(本地兜底选择|模型选择) PDF 片段 (\d+)(?:\/(\d+))? 段 \/ (\d+) 字$/, (source, selected, candidates, chars) => `${source === "模型选择" ? "Model-selected" : "Locally selected fallback"} PDF passages: ${selected}${candidates ? `/${candidates}` : ""} / ${chars} characters`],
    [/^模型查看 PDF 候选 (\d+) 段，最终未发送片段$/, (count) => `Model inspected ${count} candidate PDF passage(s); none were sent`],
    [/^模型请求 Reader PDF 文本 (.+)$/, (detail) => `Model requested Reader PDF text: ${detail.replaceAll("字", " characters").replaceAll("（已截断）", " (truncated)")}`],
    [/^模型请求远程 arXiv 论文文本 (.+)$/, (detail) => `Model requested remote arXiv paper text: ${detail.replaceAll("字", " characters").replaceAll("（已截断）", " (truncated)")}`],
    [/^已随本轮发送 PDF 全文 (.+)$/, (detail) => `Sent full PDF text: ${detail.replaceAll("字", " characters").replaceAll("（已截断）", " (truncated)")}`],
    [/^模型调用 Zotero 工具 (\d+) 次 \/ 完成 (\d+) \/ 错误 (\d+)$/, (total, completed, errors) => `Model called Zotero tools ${total} time(s) / ${completed} completed / ${errors} failed`],
    [/^本轮未请求新论文正文；保留最近上下文 (\d+) 段 \/ (\d+) 字$/, (count, chars) => `No new paper text requested; retained ${count} recent context passage(s) / ${chars} characters`],
  ];
...
export function localizeUiTree(
  ...
  while (node) {
    if (node.nodeType === view.Node.TEXT_NODE) localizeTextNode(node as Text, language);
  ...
  const observer = new MutationObserverCtor((records: MutationRecord[]) => {
    ...
    for (const record of records) {
      if (record.type === "characterData") {
        ...
        if (priorSource != null && text.data !== uiText(priorSource, language)) {
