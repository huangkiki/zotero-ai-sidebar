...
export function normalizeUiSettings(value: unknown): UiSettings {
  const input =
    value && typeof value === "object" ? (value as Partial<UiSettings>) : {};
  return {
    messageActionsPosition: isMessageActionsPosition(
    ...
    userProfile: normalizeProfile(
      input.userProfile,
      DEFAULT_UI_SETTINGS.userProfile,
    ),
...
function isMessageActionsPosition(
  value: unknown,
): value is MessageActionsPosition {
