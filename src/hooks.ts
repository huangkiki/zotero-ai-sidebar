...
function setupPreferencesPane(win: Window): void {
  ...
  byID<HTMLButtonElement>(doc, "zai-prompt-save")?.addEventListener(
    "click",
    () => {
  ...
  byID<HTMLButtonElement>(doc, "zai-tool-save")?.addEventListener(
    "click",
    () => {
  ...
  byID<HTMLButtonElement>(doc, "zai-color-save")?.addEventListener(
    "click",
    () => {
      const settings = readToolSettingsControls(doc);
      saveToolSettings(zoteroPrefs(), settings);
  ...
  byID<HTMLButtonElement>(
    ...
    flashButton(
      byID<HTMLButtonElement>(doc, "zai-text-annotation-font-save"),
      "已保存",
    );
  });
  ...
  byID<HTMLButtonElement>(doc, "zai-sync-save")?.addEventListener(
    "click",
    () => {
...
async function savePresetControlsWithConnectivity(
  ...
  const needsTest = rawPresets.filter((preset) => {
    ...
    return (
      !prior ||
      presetConnectivitySignature(prior) !== presetConnectivitySignature(preset)
    );
  });
  if (needsTest.length) {
    setStatus(
      doc,
      "zai-preset-status",
      `正在测试 ${needsTest.length} 个新增/变更配置；通过后保存...`,
    );
  } else {
...
function selectionQuestionAnnotationControl(
  ...
  head.append(labelWrap(doc, checkbox, "普通选区提问后生成建议注释"), save);
  wrap.append(
    head,
    el(
      doc,
...
function savePromptControls(
  doc: Document,
  okMessage = "提示词已保存，侧边栏按钮立即刷新。",
): void {
