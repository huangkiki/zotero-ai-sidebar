...
describe("translate cache", () => {
  ...
  it("prefers a full-text paragraph cache over a point-translation cache", () => {
    const prefs = makePrefs();
    const paragraph = "The cached paragraph should be reused when clicked.";
    setCachedTranslation(
      prefs,
      cacheKey({
        sentence: paragraph,
    ...
    const got = getParagraphCachedTranslation(prefs, {
      sentence: paragraph,
      target: "zh",
      endpoint: "https://api.example.com",
      model: "gpt-5.4",
    ...
    expect(got?.text).toBe("全文缓存。");
  });

  it("reuses full-text paragraph cache after translation settings change", () => {
    const prefs = makePrefs();
    const paragraph = "The previous full text translation should still appear.";
    setCachedTranslation(prefs, "old-full-text", {
      text: "之前全文翻译的段落。",
      model: "gpt-5.4",
      createdAt: 1000,
      sourceText: paragraph,
    ...
    const got = getParagraphCachedTranslation(prefs, {
      sentence: paragraph,
      target: "zh",
      endpoint: "https://api.new.example.com",
      model: "gpt-5.5",
      thinking: "medium",
      ctxLevel: "paragraph",
    });

    expect(got?.text).toBe("之前全文翻译的段落。");
  });
