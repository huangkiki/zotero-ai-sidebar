...
import {
  ...
  formatKeybinding,
  matchesKeybinding,
} from "../../src/translate/keybinding";

describe("keybinding", () => {
  it("parses Shift+Enter", () => {
    expect(parseKeybinding("Shift+Enter")).toEqual({
      key: "Enter",
      shift: true,
      ctrl: false,
      alt: false,
  ...
  it("formats round-trip", () => {
    expect(formatKeybinding(parseKeybinding("Ctrl+Shift+ArrowDown")!)).toBe(
...
import {
  splitSentences,
  sentenceAt,
} from "../../src/translate/sentence-splitter";
...
if (snapshot.translateCache)
  saveTranslateCacheState(prefs, snapshot.translateCache);
```

Add the helper at the bottom of the file:

```ts
function normalizeTranslateCache(
  value: unknown,
): TranslateCacheState | undefined {
...
export async function detectSentenceAtPoint(
  ...
  const normalizedOffset = mapOriginalToNormalized(
  ...
  const allSentencesNormalized = (
    await import("./sentence-splitter")
  ).splitSentences(bundle.normalizedText);
  const idx = allSentencesNormalized.findIndex(
...
function findTextLayerAncestor(node: Node): HTMLElement | null {
  ...
  while (cur && cur !== cur.ownerDocument) {
    if (cur instanceof HTMLElement && cur.classList.contains("textLayer"))
      return cur;
    cur = cur.parentNode;
  }
  return null;
...
function approxClickOffset(
  ...
  let offset = 0;
  let found = false;
  const walker = textLayer.ownerDocument.createTreeWalker(
    textLayer,
...
function mapOriginalToNormalized(
...
export function mountOverlay(input: MountOverlayInput): OverlayHandle {
  ...
  actionsRow.appendChild(
    makeBtn(iframeDoc, "💾", "保存到笔记", actions.onSave),
  );
...
function positionOverlay(
  ...
  const [pdfW, pdfH] = parseViewBox(viewBoxAttr) ?? [
    pageRect.width,
    pageRect.height,
  ];
...
export class TranslateModeController {
  ...
  disable(): void {
    const win = this.ctx.reader._internalReader?._primaryView?._iframeWindow;
    if (win && this.clickHandler)
      win.removeEventListener("click", this.clickHandler, true);
    if (win && this.keyHandler)
      win.removeEventListener("keydown", this.keyHandler, true);
  ...
  private async jump(delta: number): Promise<void> {
    ...
    if (targetIndex < 0 || targetIndex >= this.current.pageSentenceCount)
      return;
    const win = this.ctx.reader._internalReader?._primaryView?._iframeWindow;
    if (!win) return;
  ...
  private async renderForCurrent(): Promise<void> {
    ...
    for await (const chunk of translateSentence({
      sentence: this.current.text,
      paragraphContext:
        settings.ctxLevel === "paragraph"
          ? this.current.paragraphContext
          : undefined,
      preset,
      model,
      thinking: settings.thinking,
...
```ts
const translateBtn = document.createElement(
  "toolbarbutton",
...
```ts
function getOrCreateTranslateController(
  win: Window,
): TranslateModeController | null {
...
.zai-translate-overlay--error .zai-translate-overlay__body {
  color: #b3261e;
}
...
.zai-translate-overlay__btn:disabled {
  opacity: 0.4;
  cursor: default;
}
